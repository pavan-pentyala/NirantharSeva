...
**67. Fixing observation 66 (start a demo-scale scheduler in CI) introduced exactly the failure mode observation 60 already named, one push later.** Starting the demo-scale scheduler before the *whole* `npx playwright test` invocation fixed the two tests that needed it, but also escalated every other spec file's own fixture referrals for the full ~4-minute suite duration — caught on the very next push (a docs-only commit, no code change), when `dashboard.spec.ts`'s own `getByText("Not travelled yet")` locator, not scoped tightly enough, matched seven elements instead of one: five stray overdue pills belonging to unrelated, concurrently-running tests' own fixtures (`Kavita Joshi`, `Resume After Kill`, `Lakshmi Devi`, `Airplane Mode Fixture`), on top of the two legitimate matches. Fixed by splitting the suite into two invocations — `npx playwright test --grep breach` (just the two breach-titled tests, scheduler on, running first so nothing else has created a fixture yet to accidentally escalate) then `npx playwright test --grep-invert breach` (the remaining nine, scheduler stopped, real scale) — verified locally with `--list` before pushing (2 + 9 = 11, no gaps, no overlap), then confirmed green **twice in a row** in CI, not just once, before trusting it. Same root cause as observation 60, now hit twice in one session: a demo-scale timing constant applies to everything matching its precondition, not just the row it was introduced for — and the fix for the first occurrence (P9.1's demo script) does not automatically transfer awareness to the second one (CI's own test run) without deliberately re-applying the same reasoning.

**68. A React Router navigation leaves the old route's DOM transiently attached for a moment after the URL changes, and an unscoped `getByText` assertion right after a click can catch that moment — a real race, not a demo-scale-scheduler artifact, that surfaced only because CI finally reached this test for the first time.** `dashboard.spec.ts`'s "a new breach appears..." test clicks a referral row (`ashaRow`) to navigate to its detail page, confirms the URL changed, then checks `page.getByText("Not travelled yet")` — unscoped, unlike the identical check four lines earlier which is correctly scoped to `ashaRow`. The seeded fixture patient Lakshmi Devi's own referral is also `CREATED`, so it always shows the same "Not travelled yet" label regardless of escalation state — if her list row is still attached when the assertion runs (a genuine, if narrow, timing window between the URL changing and React actually unmounting the old route), Playwright's strict mode throws on the ambiguous match. Reproduced locally, deterministically, on the first attempt, by matching CI's exact conditions (fresh seed, demo-scale scheduler running). The `error-context.md` snapshot Playwright captures on failure showed only ONE match at that instant — confirming the second match happened during an earlier poll within the same 5-second window, not a stable double-render. Fixed at the cause, not the symptom: `await expect(ashaRow).toBeHidden()` before the detail-page check, waiting for the old route to actually detach rather than narrowing the locator to paper over the timing. Verified with a fresh database both times, twice standalone and once as part of the full 11-test suite split exactly as CI runs it. The demo-scale scheduler (observations 66-67) didn't cause this race — it almost certainly made it far more likely to land inside a single test run, by generating enough extra background sync traffic that the transition window was hit more often — which is the most likely reason nothing had caught it before: this test had never actually completed in CI until this session's `clock-discipline` fix let it run for the first time.

**69. `api` and `scheduler` sharing the `server_venv` named volume can race Docker's own populate-from-image step on a clean runner, and it is a real race, not flakiness — reproduced byte-for-byte identically twice in a row.** `docker-compose.yml`'s `api` and `scheduler` services both mount the same named volume at `/app/.venv` (deliberately — same image, same dependencies, so a dependency change only needs installing once). On a runner with no prior state, that volume is empty, and Docker populates it from the image's own `/app/.venv` on first mount — including recreating the base image's `lib64 -> lib` symlink. If two containers first-mount the same empty volume in the same `docker compose up`, that populate step can run concurrently for both, and the second attempt to create the same symlink fails with "file exists," aborting the whole `up`. Fixed by sequencing: `docker compose up -d --build api` first (its own `depends_on: db` already serialises db-then-api, and Docker's populate-from-image happens atomically at container creation, before the entrypoint runs — so by the time this command returns, the volume is guaranteed fully populated), then `docker compose up -d --build` for everything else, so `scheduler`'s later mount finds an already-populated volume with nothing left to race for. Confirmed green three times in a row after the fix, on a workflow that had failed identically twice before it. General lesson: two containers sharing a named volume for legitimate reasons (avoiding duplicate installs) inherits a real "who populates it first" race on any environment where that volume starts empty — CI's fresh runners hit this every time in a way a long-lived local Docker Desktop volume never would, because locally the volume is essentially always already populated from a previous run.
