...
**A fix for one CI bug reintroduced a lesson already learned earlier the same session, one push later.** Starting a demo-scale scheduler in CI's `e2e` job (to fix observation 66) escalated every test file's own fixture referrals for the whole suite duration, not just the two tests that needed it — the exact same "a fast-escalation setting applies to everything matching its precondition, not just the row you meant" finding from P9.1 (observation 60), now recurring in a completely different context (CI test isolation rather than a live demo) within the same session. Caught on the very next push and fixed by isolating the two dependent tests into their own `--grep`-scoped run, verified locally before pushing and then confirmed green twice in a row in CI, not once, given the session had already been burned by trusting a single green run earlier. Worth a sentence in the report specifically about this recurrence: naming a lesson once (even correctly, even in writing) doesn't automatically prevent the same shape of mistake in a different context minutes later — the discipline that actually caught it was re-verifying the fix's own side effects, not having already written the lesson down.

**A third, genuinely pre-existing test race was also found this session, one push after the second CI fix — and this one predates the whole CI audit entirely.** `dashboard.spec.ts`'s breach test had an unscoped `getByText` assertion sitting right after a client-side route change, which can catch the old route's DOM in the brief window before React actually unmounts it. This bug has nothing to do with any of this session's CI changes — it was always latent in the test file, present since whenever this test was first written, and simply never triggered because CI had never reached this test at all before today. Reproduced deterministically on the first local attempt by matching CI's exact conditions, and fixed at its actual cause (wait for the old row to detach) rather than by narrowing the locator to hide the symptom. Worth a sentence in the report's discussion of test reliability: a CI pipeline that has never run to completion isn't merely "unverified" — it can be actively hiding a backlog of real, fixable test bugs that no amount of passing local test runs would surface, because local runs of this same file, on this same session's own machine, over many prior sessions, apparently never hit this exact race either. Three real, independent CI-related fixes in one short investigation (the docstring greps, the missing scheduler, this navigation race) is itself a data point worth reporting honestly: "check CI" was not a formality this session, it materially changed what could be claimed as verified.

**A fifth finding, this time a genuine infrastructure race rather than an application or test bug: two containers sharing a named volume for a legitimate reason (avoiding a duplicate dependency install) can race Docker's own populate-from-image step the first time that volume is ever mounted, on any environment where it starts empty.** Reproduced identically twice on GitHub's runners (never on this session's own long-lived local Docker Desktop volume, which had already been populated by earlier work). Fixed by sequencing the two containers' first starts rather than letting them race, then confirmed green three times in a row on a workflow that had failed the same way twice before the fix. Worth a sentence in the report specifically distinguishing this from the other four findings: the docstring greps, the missing scheduler, its contamination side effect, and the navigation race were all things a careful review of the diff could plausibly have caught by reading; this one could not have been — it only exists on a machine whose volumes start genuinely empty, which is CI and only CI. A local-only development discipline, however careful, structurally cannot audit for this class of bug.
